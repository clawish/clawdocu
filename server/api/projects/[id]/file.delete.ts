...
export default defineEventHandler(async (event) => {
  ...
  if (Array.isArray(itemData)) {
    ...
    // Filter files that are inside the directory
    const filesToDelete = treeData.tree.filter((item: any) => 
      item.type === 'blob' && item.path.startsWith(filePath + '/')
    )
    ...
    // Create new tree with files deleted (sha: null)
    const newTree = filesToDelete.map((item: any) => ({
      path: item.path,
      mode: item.mode,
      type: item.type,
      sha: null
